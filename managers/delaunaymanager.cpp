                ...
                cg3::Point2Dd(BOUNDINGBOX, BOUNDINGBOX))
{
    ...
    //Inserisco nel vettore points i punti del bounding triangle
    points.push_back(BT_P1);
    points.push_back(BT_P2);
    points.push_back(BT_P3);

    //Metodo che crea il nodo e il triangolo corrispondente al bounding triangle
    triangulation.insertBoundingTriangle(BT_P1,BT_P2, BT_P3);

    //Inizialmente si suppone di non visualizzare il bounding Triangle
    delaunay.isVisible(false);

    //Non viene visualizzato inizialmente voronoi
    voronoi.isVisible(false);

    //Prende i nodi del vettore così da stamparli
    delaunay.takesNodes(triangulation.getDag().getVector());

    //push degli oggetti drawable e aggiornamento della canvas
    mainWindow.pushObj(&delaunay,"Delaunay Trianglulation");
    mainWindow.pushObj(&voronoi,"Voronoi Diagram");
    mainWindow.updateGlCanvas();
    fitScene();
...
DelaunayManager::~DelaunayManager() {
    ...
    //Cancellazione della triangolazione e degli oggetti drawable
    triangulation.clear();
    mainWindow.deleteObj(&delaunay);
...
void DelaunayManager::computeDelaunayTriangulation(const std::vector<cg3::Point2Dd>& inputPoints) {
    ...
    for(unsigned long i=0;i<inputPoints.size();i++) //Si scorre il vettore di punti inserendoli nella triangolazione
        triangulation.insertPointToTriangulation(inputPoints[i], false);
...
void DelaunayManager::addPointToDelaunayTriangulation(const cg3::Point2Dd& p) {
    ...
    /* WRITE YOUR CODE HERE! Read carefully the above comments! This line can be deleted */
        ...
        points.push_back(p); //inserisce il punto nel vettore points

        //Inserisce il punto nella triangolazione
        triangulation.insertPointToTriangulation(p, true);

        //Disegna la triangolazione con i nuovi triangoli
...
void DelaunayManager::clearDelaunayTriangulation() {
    ...
    //Cancellazione della triangolazione e della mappa contenente l'associazione punto-indice
    triangulation.clear();
    this->indexPoint.clear();
...
void DelaunayManager::drawDelaunayTriangulation() {
    ...
    /* WRITE YOUR CODE HERE! Read carefully the above comments! This line can be deleted */
    delaunay.takesNodes(triangulation.getDag().getVector()); //disegno i triangoli con il vettore aggiornato
    voronoi.takeNodes(triangulation.getDag().getVector());//disegno voronoi con il vettore aggiornato
    fitScene();
    /********************************************************************************************************************/
...
void DelaunayManager::eraseDrawnDelaunayTriangulation() {
    ...
    //Cancello i nodi (dove a ognuno dei quali è associato un triangolo) dai rispettivi vettori di delaunay e voronoi
    delaunay.clearDagNode();
    voronoi.clearDagNode();

    //Setto di nuovo il bounding triangle così da poter fare una nuova triangolazione
    triangulation.insertBoundingTriangle(BT_P1,BT_P2, BT_P3);

    //Passo il vettore di nodi (e quindi di triangoli) aggiornato
    delaunay.takesNodes(triangulation.getDag().getVector());
...
void DelaunayManager::setVisibilityBoundingTriangle(const bool visible)
{
    ...
    delaunay.isVisible(visible); //se si fa check si vuole visualizzare il bounding triangle
...
void DelaunayManager::checkTriangulation() {
    ...
    //Inserisco i punti del bounding triangle nel vettore points
    points.push_back(BT_P1);
    points.push_back(BT_P2);
    points.push_back(BT_P3);

    unsigned long i=0;
    for( i=0; i<this->points.size();i++) //uso this->points e inserisco nel vettore points i punti dell'altro vettore
    {

        points.push_back(this->points.at(i));
    }

    for(unsigned long i=0; i<points.size();i++) //scorro il vettore dei punti
    {
        //inserisco nella mappa il punto con il suo relativo indice
         indexPoint.insert(std::pair<cg3::Point2Dd, unsigned int>(points.at(i),i));

    }
    triangles.resize(triangulation.getVector().size(),3); //setto la dimensione della matrice

    //scorro il vettore di nodi (contenente ognuno un triangolo) e inserisco nella matrice l'indice dei punti
    //della mappa del triangolo i-esimo
    for(unsigned long i=0;i<triangulation.getVector().size();i++)
    {
        if(triangulation.getVector().at(i).getSalta()==false)
        {
            triangles(k,0)=indexPoint.find(triangulation.getVector().at(i).get_Triangle().getFirstPoint())->second;
            triangles(k,1)=indexPoint.find(triangulation.getVector().at(i).get_Triangle().getSecondPoint())->second;
            triangles(k,2)=indexPoint.find(triangulation.getVector().at(i).get_Triangle().getThirdPoint())->second;
            k++;
        }
    }
...
{
    voronoi.isVisible(true);
    voronoi.takeNodes(triangulation.getDag().getVector());
    mainWindow.updateGlCanvas();
}
 ...
 * on the bounding box in which the points can be addedelaunay.
 */
